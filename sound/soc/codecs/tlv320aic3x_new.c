...
#include "tlv320aic3x_new.h"

/* add by achang, use external supply and reset pin in our EVB */
#define EXT_SUPPLY_AND_RESET
...
static const struct reg_default init_reg[] = {
	...
	/* INPUT */
	{LINE1L_2_LADC_CTRL,      0x78},
	{LINE1R_2_LADC_CTRL,      0x00},
	{LINE1L_2_RADC_CTRL,      0x78},
	{LINE1R_2_RADC_CTRL,      0x78},
					...
					struct snd_ctl_elem_value *ucontrol)
{
	...
	struct snd_soc_dapm_context *dapm = snd_soc_component_get_dapm(&codec->component);//snd_soc_codec_get_dapm(codec);//achang
...
static const struct snd_soc_dapm_route intercon[] = {
	/* Input */
	{"LINE1L", NULL, "Mic Bias"},//achang 
	{"LINE1R", NULL, "Mic Bias"},//achang
...
{
	struct aic3x_priv *aic3x = snd_soc_codec_get_drvdata(codec);
	struct snd_soc_dapm_context *dapm = snd_soc_component_get_dapm(&codec->component);//snd_soc_codec_get_dapm(codec);
			   ...
			   struct snd_soc_dai *dai)
{
	...
	if (params_channels(params) == 1) {
		data = (LDAC2MONOMIX | RDAC2MONOMIX); //achang
	} else {
...
static int aic3x_mute(struct snd_soc_dai *dai, int mute)
{
	...
	if (mute) {
		...
		/* add by achang, for pop noise */
		snd_soc_update_bits(codec, HPLOUT_CTRL, UNMUTE, 0);
		snd_soc_update_bits(codec, HPROUT_CTRL, UNMUTE, 0);
		snd_soc_update_bits(codec, LLOPM_CTRL, UNMUTE, 0);
		snd_soc_update_bits(codec, RLOPM_CTRL, UNMUTE, 0);
	} else {
...
#ifndef EXT_SUPPLY_AND_RESET //achang
...
static int aic3x_init(struct snd_soc_codec *codec)
{
	...
	/* unmute all outputs */
#ifndef EXT_SUPPLY_AND_RESET //achang, don't unmute here, in case pop noise.
	snd_soc_update_bits(codec, LLOPM_CTRL, UNMUTE, UNMUTE);
	snd_soc_update_bits(codec, RLOPM_CTRL, UNMUTE, UNMUTE);
	snd_soc_update_bits(codec, HPLOUT_CTRL, UNMUTE, UNMUTE);
	snd_soc_update_bits(codec, HPROUT_CTRL, UNMUTE, UNMUTE);
	...
	case AIC3X_MODEL_3007:
		...
		break;
	/* add by achang, Initialize some registers */
	case AIC3X_MODEL_3104:
		regmap_multi_reg_write(aic3x->regmap, init_reg, ARRAY_SIZE(init_reg));
		break;
	}
...
//static const struct reg_sequence aic3007_class_d[] = { //achang
 ...
 * around
 */
extern void quec_set_codec_info(const char* codec_name, const char* dai_name); //achang
			   ...
			   const struct i2c_device_id *id)
{
	...
	// add by achang, for codec compatible dynamically. (start)
	regcache_cache_bypass(aic3x->regmap, true);
	ret = regmap_read(aic3x->regmap, 0xff, &reg);
	if (ret < 0 || reg == 0x42) {
		dev_err(&i2c->dev, "Failed to read 0xff reg: %d\n", ret);
		return ret;
	}
	ret = regmap_read(aic3x->regmap, 0x0, &reg);
	if (ret < 0) {
		dev_err(&i2c->dev, "Failed to read 0x0 reg: %d\n", ret);
		return ret;
	}
	regcache_cache_bypass(aic3x->regmap, false);
	// add by achang, for codec compatible dynamically. (end)
	...
	quec_set_codec_info("tlv320aic3x-codec.4-0018", "tlv320aic3x-hifi");	

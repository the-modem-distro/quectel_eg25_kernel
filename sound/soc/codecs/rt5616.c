...
static struct rt5616_init_reg init_list[] = {
	...
	//grady,2018-5-22, to improve DAC signal-noise ratio
	//{ RT5616_ADDA_CLK1, 	0x0014 } ,
	{ RT5616_ADDA_CLK1, 	0x0000 } ,
	...
	struct snd_kcontrol *kcontrol, int event)
{
	...
	case SND_SOC_DAPM_POST_PMU:
		...
		if(1 == alc_pwrctr)
		{
			printk("=========%s,alc_pwrctr=%d.========\n", __func__, alc_pwrctr);
			snd_soc_write(codec, RT5616_PWR_VOL, 0x3F00);
		}
		// 2017.10.24 eddy.qin comment
		//rt5616_index_write(codec, RT5616_LOUT_CTRL1, 0x0808);
		//snd_soc_write(codec, RT5616_HP_VOL, 0x1010); // add hertz 20170511//20170809, comment out by jun.wu, config alc5616 volume to default value
		// end eddy.qin 
		break;

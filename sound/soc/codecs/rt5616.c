...
static ssize_t rt5616_detected_state_show(struct device *dev, struct device_attribute *attr, char *buf)
{
	int cnt = 0;
...
static DEVICE_ATTR(rt5616_detected_state, S_IRUGO, rt5616_detected_state_show, NULL);
...
static int rt5616_probe(struct snd_soc_codec *codec)
{
    ...
    ret = device_create_file(codec->dev, &dev_attr_rt5616_detected_state);
    if (ret != 0) {
        dev_err(codec->dev,
        "Failed to create detected sysfs files: %d\n", ret);
        return ret;
    }
	return 0;
}
			    ...
			    const struct i2c_device_id *id)
{
	...
	if (val != 0x6281) {
		...
		dev_info(&i2c->dev, "Device with ID register %#x *IS* rt5616\n", val);
	...
	pr_info("%s: rt5616.2-001b rt5616-aif1\n", __func__);

...
static const struct reg_default init_reg[] = {
	{ NAU8810_REG_DAC, 0x0008 },
	{ NAU8810_REG_ADC, 0x0108 },
	{ NAU8810_REG_ADCBOOST, 0x0000 },
};
...
static const struct snd_soc_dapm_widget nau8810_dapm_widgets[] = {
	...
	SND_SOC_DAPM_MIXER("Input Boost Stage", NAU8810_REG_POWER2,
		NAU8810_BST_EN_SFT, 0, nau8810_mic_boost_controls,
...
static const struct snd_soc_dapm_route nau8810_dapm_routes[] = {
	...
	{"Input Boost Stage", "Mic Volume", "MICP"},
	...
	enum snd_soc_bias_level level)
{
	...
	case SND_SOC_BIAS_STANDBY:
		regmap_update_bits(map, NAU8810_REG_POWER1,
			NAU8810_IOBUF_EN | NAU8810_ABIAS_EN, 0);
...
// codec_reg dedug file
static ssize_t codec_reg_store(struct device *dev,
		struct device_attribute *attr, 
		const char *buf, size_t count)
{
	struct i2c_client *client = to_i2c_client(dev);
	struct nau8810 *nau8810 = i2c_get_clientdata(client);
	char *start = (char *)buf;
	unsigned int reg;
	unsigned long val;
	int ret;
	...
	if (ret)
		return ret;
	pr_info("%s reg:%x val:%x\n", __func__, reg, (unsigned int)val);
	regmap_write(nau8810->regmap, reg, (unsigned int)val);
	return count;
}

static unsigned int nau8810_reg_read(struct device *dev,unsigned int reg)
{
	struct i2c_client *i2c = to_i2c_client(dev);
	struct i2c_msg xfer[2];
	unsigned int reg_tmp = reg, val = 0;
	int ret;
		...
		struct device_attribute *attr, char *buf)
{
	...
	for(reg = 0; reg <= NAU8810_REG_MAX; reg++) {
		if(!nau8810_readable_reg(dev, reg))
			continue;
		val = nau8810_reg_read(dev, reg);
		if (val < 0) {
			dev_err(dev, "%s read reg:%04x fail\n", __func__, reg);
			continue;
		}
		ptr += sprintf(ptr, "%02x: %04x\n", reg, val);
	}

	return ptr - buf;
}
static DEVICE_ATTR(codec_reg, 0664, codec_reg_show, codec_reg_store);
static void nau8810_create_debug_file(struct device *dev)
{
	int ret = 0;

	ret = device_create_file(dev, &dev_attr_codec_reg);
	if (ret < 0)
		dev_err(dev, "%s fail\n", __func__);
}
			    ...
			    const struct i2c_device_id *id)
{
	...
	if (val == 0x1A) {
		quec_set_codec_info("nau8810.4-001a", "nau8810-hifi");    
	} else {
		dev_err(&i2c->dev, "Failed to read device ID\n");
		return -EINVAL;
	}

	nau8810_create_debug_file(dev);

	regmap_multi_reg_write(nau8810->regmap, init_reg, ARRAY_SIZE(init_reg));

...
static int wakelock_release(struct inode *inode, struct file *file) {
	...
	//disable autosleep, avoid system suspend when user app died unexpectedly. Gale-20181021
	extern int pm_autosleep_set_state(suspend_state_t state);
	pm_autosleep_set_state(PM_SUSPEND_ON);
	pr_info("[%s]: Also disable autosleep feature, avoid system suspend when user app died unexpectedly.\n", __func__);

    ...
    while (1) {
        ...
        if (status == 0) {
			...
			//20170522 add by cullen for intest mcu spi bug
	   		 if(rev_enable == true) {
	   			ql_spidev->tx_msg->rev[0]=0;
            	ql_spidev->tx_msg->rev[1]=0;
           		ql_spidev->tx_msg->rev[2]=0;
            	ql_spidev->tx_msg->rev[3]=0;
	   		 }
            if (tx_ch_info) {
            ...
            if (status == 0) {
                ...
                if (status == 0) {
					...
					unsigned char *head_ptr, *cnt_ptr;

					msg_cnt = 0;
					head_ptr = cnt_ptr = (unsigned char *)ql_spidev->rx_msg;

					while(*(cnt_ptr+msg_cnt) != 0x5a && msg_cnt < frame_size)
					{
						msg_cnt++;
					}

					if(msg_cnt >= frame_size)
					{
						continue;
					}
					else if(msg_cnt)
                    ...
                    if (/*m.actual_length && */(ql_spidev->rx_msg->flag == 0x5a) && (QMSG_GET_CHN(ql_spidev->rx_msg) < SPI_MAX_CHN)
						...
						//20170522 add by cullen for intest mcu spi bug
								if(rev_enable == true) {
                                	ql_spidev->rx_msg->rev[0]=0;
                                	ql_spidev->rx_msg->rev[1]=0;
                                	ql_spidev->rx_msg->rev[2]=0;
                                	ql_spidev->rx_msg->rev[3]=0;

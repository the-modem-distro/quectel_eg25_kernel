...
static int msm_otg_link_clk_reset(struct msm_otg *motg, bool assert)
{
	...
	if (assert) {
		...
		dev_info(motg->phy.dev, "block_reset ASSERT\n");
		...
		dev_info(motg->phy.dev, "block_reset DEASSERT\n");
...
static int msm_otg_reset(struct usb_phy *phy)
{
	...
	if (pdata->otg_control == OTG_PHY_CONTROL) {
		...
		if (motg->phy_irq) {
			pr_err("%s: writeb_relaxed phy irq\n", __func__);
...
static void msm_otg_kick_sm_work(struct msm_otg *motg)
{
	...
	if (atomic_read(&motg->pm_suspended) &&
		...
		pr_err("%s: PM Suspended but state is not\n", __func__);
		motg->sm_work_pending = true;
	} else if (!motg->sm_work_pending) {
		pr_err("%s: We have pending work to do\n", __func__);
...
{
	struct msm_otg *motg = container_of(phy, struct msm_otg, phy);
	/* int suspend: 0 -> resume, 1 -> suspend */
	...
	if (suspend) {
		...
		pr_err("%s: UDC called for host resume or remote wakeup\n", __func__);
...
static void msm_otg_exit_phy_retention(struct msm_otg *motg)
{
	...
	pr_info("USB PHY is exited from retention\n");
...
static int msm_otg_suspend(struct msm_otg *motg)
{
	...
	if (atomic_read(&motg->in_lpm)) {
		dev_err(phy->dev, "Trying to suspend while in LPM\n");
		return 0;
	}

	cancel_delayed_work_sync(&motg->perf_vote_work);
		pr_err("%s: diable irq 4\n", __func__);

	disable_irq(motg->irq);
	if (motg->phy_irq) {
	pr_err("%s: disable phy irq 4\n", __func__);
...
lpm_start:
	...
	pr_err("%s:  enable irq 5\n", __func__);

		enable_irq(motg->irq);
		if (motg->phy_irq) {
	pr_err("%s: enable phy irq 5\n", __func__);
...
phcd_retry:
	...
	if (cnt >= phy_suspend_timeout) {
		...
		if (device_bus_suspend) {
			dev_info(phy->dev, "PHY suspend aborted\n");
			ret = -EBUSY;
			goto phy_suspend_fail;
		} else {
			if (phcd_retry_cnt++ < PHY_SUSPEND_RETRIES_MAX) {
				dev_info(phy->dev, "PHY suspend retry\n");
				goto phcd_retry;
			} else {
	...
	if (device_may_wakeup(phy->dev)) {
		...
		if (motg->phy_irq) {
			pr_err("%s: enable phy irq wake\n", __func__);
			enable_irq_wake(motg->phy_irq);
		}
		if (motg->pdata->pmic_id_irq) {
			pr_err("%s: enable pmic id irq wake\n", __func__);
			enable_irq_wake(motg->pdata->pmic_id_irq);
		}
		if (motg->ext_id_irq) {
				pr_err("%s: enable ext id irq wake\n", __func__);
	...
	/* Enable ASYNC IRQ during LPM */
		pr_err("%s: enable async irq 6\n", __func__);

	enable_irq(motg->async_irq);
		pr_err("%s: enable irq 6\n", __func__);

	enable_irq(motg->irq);
	}
	if (motg->phy_irq) {
	pr_err("%s: enable phy irq 6\n", __func__);
...
phy_suspend_fail:
	pr_err("%s: enable irq 7\n", __func__);

	enable_irq(motg->irq); 
	if (motg->phy_irq) {
	pr_err("%s: enable phy irq 7\n", __func__);
...
static int msm_otg_resume(struct msm_otg *motg)
{
	...
	if (motg->phy_irq) {
	pr_err("%s: disable phy irq 8\n", __func__);

		disable_irq(motg->phy_irq);
	}

	if (motg->host_bus_suspend || motg->device_bus_suspend) {
		pr_err("%s: disable irq 8\n", __func__);
...
skip_phy_resume:
	...
	if (device_may_wakeup(phy->dev)) {
		...
		if (motg->phy_irq) {
	pr_err("%s: disable phy irq wake 9\n", __func__);

			disable_irq_wake(motg->phy_irq);
		}
		if (motg->pdata->pmic_id_irq) {
	pr_err("%s: disable pmic id irq 9\n", __func__);

			disable_irq_wake(motg->pdata->pmic_id_irq);
		}
		if (motg->ext_id_irq) {
	pr_err("%s: disable ext id irq 9\n", __func__);
	...
	if (motg->async_int) {
		/* Match the disable_irq call from ISR */
			pr_err("%s: enable async int isr 10\n", __func__);

		enable_irq(motg->async_int);
		motg->async_int = 0;
	}
	if (motg->phy_irq) {
	pr_err("%s:  enable phy irq 10\n", __func__);

		enable_irq(motg->phy_irq);
	}
		pr_err("%s: enable irq 10\n", __func__);

	enable_irq(motg->irq);

	/* Enable ASYNC_IRQ only during LPM */
		pr_err("%s: disable async irq 11\n", __func__);
...
{
	if (!psy) {
		dev_info(motg->phy.dev, "no usb power supply registered\n");
		goto psy_error;
	}
...
static void msm_otg_set_online_status(struct msm_otg *motg)
{
	...
	if (power_supply_set_online(psy, false))
		dev_info(motg->phy.dev, "error setting power supply property\n");
}
...
static int msm_otg_set_host(struct usb_otg *otg, struct usb_bus *host)
{
	...
	otg->host = host;
	dev_info(otg->phy->dev, "host driver registered w/ tranceiver\n");
					...
					struct usb_gadget *gadget)
{
	...
	otg->gadget = gadget;
	dev_info(otg->phy->dev, "peripheral driver registered w/ tranceiver\n");
...
static void msm_otg_chg_check_timer_func(unsigned long data)
{
	...
	if (atomic_read(&motg->in_lpm) ||
		...
		dev_info(otg->phy->dev, "Nothing to do in chg_check_timer\n");
		return;
	}

	if ((readl_relaxed(USB_PORTSC) & PORTSC_LS) == PORTSC_LS) {
		dev_info(otg->phy->dev, "DCP is detected as SDP\n");
...
static void msm_chg_detect_work(struct work_struct *w)
{
	...
	dev_info(phy->dev, "chg detection work\n");
...
static void msm_otg_sm_work(struct work_struct *w)
{
	...
	if (motg->resume_pending) {
		...
		if (atomic_read(&motg->in_lpm)) {
			pr_err("%s: USB is in LPM mode, resuming\n", __func__);
			msm_otg_resume(motg);
		}
	...
	case OTG_STATE_UNDEFINED:
		...
		ret = of_platform_populate(motg->pdev->dev.of_node, NULL, NULL,
					&motg->pdev->dev);
		if (ret)
			dev_info(&motg->pdev->dev, "failed to add BAM core\n");
	...
	case OTG_STATE_B_SUSPEND:
		...
		if (!test_bit(B_SESS_VLD, &motg->inputs)) {
			...
			pr_err("%s: Resuming to peripheral\n", __func__);
			otg->phy->state = OTG_STATE_B_PERIPHERAL;
			pm_runtime_get_sync(dev);
		}
		break;
...
static irqreturn_t msm_otg_irq(int irq, void *data)
{
	...
	if ((otgsc & OTGSC_IDIS) && (otgsc & OTGSC_IDIE)) {
			pr_err("%s: IRQ: %d IDIS AND IDIE\n", __func__, irq);

		if (otgsc & OTGSC_ID) {
			dev_info(otg->phy->dev, "ID set\n");

			set_bit(ID, &motg->inputs);
		} else {
			dev_info(otg->phy->dev, "ID clear\n");
	
			clear_bit(ID, &motg->inputs);
		}
		work = 1;
	} else if ((otgsc & OTGSC_BSVIE) && (otgsc & OTGSC_BSVIS)) {
			pr_err("%s: IRQ: %d BSVIE BSVIS\n", __func__, irq);

		if (otgsc & OTGSC_BSV) {
			dev_info(otg->phy->dev, "BSV set\n");
			set_bit(B_SESS_VLD, &motg->inputs);
		} else {
			dev_info(otg->phy->dev, "BSV clear\n");
			clear_bit(B_SESS_VLD, &motg->inputs);
			clear_bit(A_BUS_SUSPEND, &motg->inputs);
		}
		work = 1;
...
static void msm_id_status_w(struct work_struct *w)
{
	...
	dev_info(motg->phy.dev, "ID status_w\n");
					...
					unsigned long mode, void *unused)
{
	...
	dev_info(motg->phy.dev, "OTG PM notify:%lx, sm_pending:%u\n", mode,
				  ...
				  const union power_supply_propval *val)
{
	...
	case POWER_SUPPLY_PROP_INPUT_CURRENT_MAX:
		...
		if (motg->chg_type != USB_INVALID_CHARGER) {
			dev_info(motg->phy.dev, "update type-c charger\n");
...
static void msm_otg_pnoc_errata_fix(struct msm_otg *motg)
{
	...
	dev_info(motg->phy.dev, "applying fix for pnoc h/w issue\n");
...
struct msm_otg_platform_data *msm_otg_dt_to_pdata(struct platform_device *pdev)
{
	...
	if (pdata->usbeth_reset_gpio < 0)
		pr_err("usbeth_reset_gpio is not available\n");
...
static int msm_otg_probe(struct platform_device *pdev)
{
	...
	if (of_property_read_u32(pdev->dev.of_node,
		...
		dev_info(&pdev->dev, "core_clk svs freq not specified\n");
	} else {
	...
	if (IS_ERR(motg->sleep_clk)) {
		...
		if (ret == -EPROBE_DEFER)
			...
			dev_info(&pdev->dev, "failed to get sleep_clk\n");
	} else {
	...
	if (!pdata->bus_scale_table)
		dev_info(&pdev->dev, "bus scaling is disabled\n");
	...
	if (pdata->enable_sec_phy) {
		res = platform_get_resource_byname(pdev,
				IORESOURCE_MEM, "tcsr");
		if (!res) {
			dev_info(&pdev->dev, "missing TCSR memory resource\n");
		} else {
			tcsr = devm_ioremap_nocache(&pdev->dev, res->start,
				resource_size(res));
			if (!tcsr) {
				dev_info(&pdev->dev, "tcsr ioremap failed\n");
			} else {
	...
	if (IS_ERR(motg->phy_pinctrl)) {
		...
		dev_info(&pdev->dev, "Target does not use pinctrl\n");
	...
	if (motg->phy_irq < 0) {
		dev_info(&pdev->dev, "phy_irq is not present\n");
		motg->phy_irq = 0;
	} else {
	...
	if (motg->pdata->mode == USB_OTG &&
		...
		if (id_irq) {
			 ...
			 */
			dev_info(&pdev->dev, "PMIC does ID detection\n");
		}
	}
...
/*	if (motg->pdata->delay_lpm_on_disconnect) {
	...
	*/
	...
	if (ret)
		dev_info(&pdev->dev, "fail to setup cdev\n");
...
static void msm_otg_shutdown(struct platform_device *pdev)
{
	...
	dev_info(&pdev->dev, "OTG shutdown\n");
	msm_hsusb_vbus_power(motg, 0);
}
...
static int msm_otg_pm_resume(struct device *dev)
{
	...
	dev_info(dev, "OTG PM resume\n");
	ret = msm_otg_resume(motg);
	if (ret)
		return ret;

	/*
	 * For whatever reason, pm_runtime_disable wasnt being
	 * called in here... ?? (bik)
	 */

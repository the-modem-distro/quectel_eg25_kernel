...
unsigned int Quectel_Set_Partition_RestoreFlag(const char * partition_name, int where)
{
	...
	else
	{
		...
		if((QUEC_BACKUP_MAGIC1 != Flag_msg.magic1) ||(QUEC_BACKUP_MAGIC2 != Flag_msg.magic2))// ˵˿һд
		{
			...
			if(0/*1 == fota_upgradedFlag_msg.fota_upgradedFlag*/)//Ramos.zhang-20181019 fota update backup partitions,not whole restore
			{
			...
			else// the module not upgraded by fota  never
			{
	            ...
	            else if(!strcmp(partition_name, "modem")) // modemĻԭflag
	            {
	                Flag_msg.modem_restore_flag = 1;
	            }
	            else if(!strcmp(partition_name, "efs2")) // efs2 ûԭmodmeǱߣûд
	            {
					Flag_msg.cefs_restore_flag=1;
	            }
			}
...
******************************************************************************************/
unsigned int Quectel_Restore(char * partition, int where)
{
	printk("Quectel Restore  mtd name = %s \n",partition);
	if(!strcmp(partition,"system")){
		Quectel_Set_Partition_RestoreFlag("system",where);
	}else if(!strcmp(partition,"modem")){
		Quectel_Set_Partition_RestoreFlag("modem",where);
	}else if(!strcmp(partition,"recoveryfs")){
		Quectel_Set_Partition_RestoreFlag("recoveryfs",where);
	}else{
		Quectel_Erase_Partition(partition);
	}
	return 0;
}

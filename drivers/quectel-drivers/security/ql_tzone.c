...
#define MDM_EFUSE_PHY_ADDR      0xa4000
#define MDM_EFUSE_SIZE          0x1000
#define MDM_BYTES_PER_FUSE_ROW  8

void __iomem *ql_tz_efuse_base = NULL;
...
static int ql_tz_fuse_read(ql_tz_fuse_val_t *fuse_val)
{
    ql_tz_fuse_val_t fuse_val_k;
    int rc;

    if (ql_tz_efuse_base == NULL) {
        pr_err("efuse base invaild");
        return -ENOMEM; 
    }

    rc = copy_from_user(&fuse_val_k, fuse_val, sizeof(ql_tz_fuse_val_t));

    if (rc < 0 || fuse_val_k.row_num > 127) {
        pr_err("read row_num = %d invalid", fuse_val_k.row_num);
        return -EINVAL; 
    }

    fuse_val_k.efuse = readq_relaxed(ql_tz_efuse_base + fuse_val_k.row_num * MDM_BYTES_PER_FUSE_ROW);
    rc = copy_to_user(fuse_val, &fuse_val_k, sizeof(ql_tz_fuse_val_t));
    pr_info("read %d row, val = 0x%llx\n\r", fuse_val_k.row_num, fuse_val_k.efuse);
    return rc;
}
...
static int long ql_tzone_unlocked_ioctl(struct file *file, unsigned int cmd, unsigned long arg)
{
    ...
    switch (cmd) {
        ...
        case QL_TZ_FUSE_SVC_READ:
            rc = ql_tz_fuse_read((ql_tz_fuse_val_t *) arg);
            break;
...
ssize_t secboot_state_show(struct class *class, struct class_attribute *attr, char *buf)
{
    uint64_t fuse_val;
    
    pr_info("class_name:%s, classattr_name:%s.\n", class->name, attr->attr.name);
    
    fuse_val = readq_relaxed(ql_tz_efuse_base + 58 * MDM_BYTES_PER_FUSE_ROW);
    
    if (fuse_val & OEM_SECURE_BOOT1_AUTH_EN) {
        sprintf(buf, "%s","auth_enable");
    }
    else {
        sprintf(buf, "%s", "auth_disable");
    }

    return strlen(buf) + 1;
}

CLASS_ATTR_RO(secboot_state);


ssize_t secboot_rootca_hash_show(struct class *class, struct class_attribute *attr, char *buf)
{
    uint64_t fuse_val;
    uint8_t hash[32];
    int i;
    ...
    for (i = 0; i < 4; i++) {
        fuse_val = readq_relaxed(ql_tz_efuse_base + (60 + i) * MDM_BYTES_PER_FUSE_ROW);
        memcpy(&hash[i * 7], &fuse_val, 7);
    }
    fuse_val = readq_relaxed(ql_tz_efuse_base + 64 * MDM_BYTES_PER_FUSE_ROW);
...
CLASS_ATTR_RO(secboot_rootca_hash);
...
static int __init tzone_init(void)
{
    ...
    ql_tz_efuse_base = ioremap(MDM_EFUSE_PHY_ADDR, 0x1000);
    if (ql_tz_efuse_base == NULL) {
        pr_err("[QL_TZ] ioremap efuse addr failed");
    }
    
    ql_security_class_create_file(&class_attr_secboot_state);
    ql_security_class_create_file(&class_attr_secboot_rootca_hash);
...
static void __exit tzone_exit(void)
{
    ...
    ql_security_class_remove_file(&class_attr_secboot_state);
    ql_security_class_remove_file(&class_attr_secboot_rootca_hash);
}
